...
`);
            ...
            let res = pxtc.assemble(opts.target, bin, vmsource, cres);
...
${output}</xml>`;
                ...
                function getDraggableReporterBlock(varName, varType, draggable) {
                ...
                function getCallStatement(node, asExpression) {
                    ...
                    if (info.qName == "Math.pow") {
                        ...
                        if (info.qName === "colorHelpers.hex") {
                            ...
                            r.inputs = node.arguments.map((arg, index) => mkValue("INPUT" + index, getOutputBlock(arg), colorPickerNumber));
                        }
            ...
            function checkStatement(node, env, asExpression = false, topLevel = false) {
                ...
                function checkCall(n, env, asExpression = false, topLevel = false) {
                    ...
                    if (!asExpression) {
                        ...
                        return pxtc.Util.lf("Function with forceStatement cannot be used as an expression.");
                    }
                    ...
                    function checkPredicate(p) {
                        ...
                        const predicate = p;
                        if (isOutputExpression(predicate.body, env)) {
                            return true;
                        }
            ...
            function checkExpression(n, env) {
                switch (n.kind) {
                    ...
                    case SK.Identifier:
                        ...
                        else if (isDeclaredElsewhere(n) && !(pInfo.commentAttrs && pInfo.commentAttrs.blockIdentity && pInfo.commentAttrs.enumIdentity && env.blocks.apis.byQName[pInfo.commentAttrs.blockIdentity])) {
            ...
            function isOutputExpression(expr, env) {
                switch (expr.kind) {
                    case SK.BinaryExpression: {
            ...
            function shouldEmitShadowOnly(n, blocksInfo) {
                var _a, _b;
                ...
                if (n.value.kind === "expr") {
                    if (n.value.type !== n.shadowType) {
                        ...
                        let shadowBlockShimType;
                        let shadowFieldName;
                        if (((_a = shadowBlockInfo === null || shadowBlockInfo === void 0 ? void 0 : shadowBlockInfo.attributes) === null || _a === void 0 ? void 0 : _a.shim) === "TD_ID") {
                            const argType = (_b = shadowBlockInfo.parameters[0]) === null || _b === void 0 ? void 0 : _b.type;
                        ...
                        if (shadowBlockShimType === "boolean" && isBooleanBlockType(n.value.type) ||
            ...
            function isBooleanBlockType(type) {
                return type === booleanType;
            }
            function isStringBlockType(type) {
                return type === stringType || type === colorPickerString;
            }
            function gridLiteralValue(attrs) {
                return attrs.gridLiteral || attrs.imageLiteral || attrs.colorGridLiteral;
            }
            function isColorPickerFunction(info) {
                switch (info === null || info === void 0 ? void 0 : info.qName) {
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        (function (ir) {
            ...
            function inlineSubst(e, isRtCallArg) {
                e = Expr.clone(e);
                switch (e.exprKind) {
                    case EK.PointerLiteral:
                    case EK.NumberLiteral:
    ...
    (function (pxtc) {
        ...
        function compileBinary(program, opts, res, entryPoint) {
            ...
            if (opts.target.isNative) {
                ...
                pxtc.hexfile.setupFor(optstarget, extinfo || pxtc.emptyExtInfo(), res);
...
`);
            ...
            function markUsed(decl) {
                ...
                if (opts.computeUsedParts && ((_a = pinfo.commentAttrs) === null || _a === void 0 ? void 0 : _a.parts)) {
            ...
            function emitShim(decl, node, args, shimOverride = null) {
                let attrs = parseComments(decl);
            ...
            function emitCallCore(node, funcExpr, callArgs, sig, decl = null, recv = null) {
                ...
                if (hasRecv) {
                    ...
                    if (attrs.shim && !hasShimDummy(decl)) {
                        let shimOverride = undefined;
                        if (node.kind === pxtc.SK.PropertyAccessExpression && args.length > 1) {
...
(function (ts) {
    ...
    (function (pxtc) {
        function getTsCompilerOptions(opts) {
            ...
            return Object.assign(Object.assign({}, options), opts === null || opts === void 0 ? void 0 : opts.tsCompileOptions);
        }
        ...
        function getTSProgram(opts, old) {
            ...
            let host = {
                getSourceFile: (fn, v, err) => {
                    ...
                    return ts.createSourceFile(fn, text, v, setParentNodes, undefined /* scriptKind */, options);
                },
                fileExists: fn => {
                    fn = normalizePath(fn);
                    return fileText.hasOwnProperty(fn);
    ...
    :00000001FF
        ...
        */
        ...
        (function (hexfile) {
            ...
            function getTopFlashAddress() {
                return ctx.topFlashAddr;
            }
            hexfile.getTopFlashAddress = getTopFlashAddress;
            // utility function
            ...
            function setupFor(opts, extInfo, cres) {
...
`;
        ...
        function throwAssemblerErrors(b) {
            if (b.errors.length > 0) {
                ...
                if (userErrors) {
                    //TODO
                    pxt.log(pxtc.U.lf("errors in inline assembly"));
                    pxt.log(userErrors);
        ...
        function getFlashUsableEnd(target, cres) {
        ...
        let peepDbg = false;
        function assemble(target, bin, src, cres) {
            let b = mkProcessorFile(target);
            b.emit(src);
...
`;
        ...
        function processorEmit(bin, opts, cres) {
            ...
            hexfile.setupFor(opts.target, opts.extinfo || pxtc.emptyExtInfo(), cres);
            assembleAndPatch(src, bin, opts, cres);
            if (!cres.builtVariants) {
                cres.builtVariants = [];
            }
            cres.builtVariants.push((_a = opts.extinfo) === null || _a === void 0 ? void 0 : _a.appVariant);
            ...
            if (otherVariants.length)
                try {
                    for (let other of otherVariants) {
                        ...
                        cres.builtVariants.push((_b = other.extinfo) === null || _b === void 0 ? void 0 : _b.appVariant);
                    }
                }
                finally {
...
(function (ts) {
    var pxtc;
    (function (pxtc) {
        // Candidate multipliers for the per-class interface-table hash search in
        // computeHashMultiplier (see hexfile.ts). This is a compile-time-only
        // search space: only the single chosen multiplier per class is emitted, so
        // the size of this pool has no effect on output size - more candidates just
        // give the search more chances to pack a class's table into a smaller
        // power-of-two.
        //
        // 512 primes spread across the ~15M-24M range used by the original seeds;
        // the high bits drive the hash, so an even spread matters. The first 50 are
        // the historical hand-picked seeds; the rest are evenly spaced primes.
        pxtc.primes = [
            ...
            19506731, 21454393, 23224541, 23431973, 23745511, 15000017, 15019493, 15038963, 15058457,
            15077947, 15097427, 15116891, 15136393, 15155849, 15175333, 15194819, 15214291, 15233791,
            15253253, 15272729, 15292213, 15311689, 15331187, 15350653, 15370129, 15389617, 15409099,
            15428573, 15448057, 15467533, 15487019, 15506497, 15526001, 15545459, 15564937, 15584417,
            15603901, 15623383, 15642881, 15662357, 15681823, 15701311, 15720779, 15740261, 15759763,
            15779221, 15798743, 15818197, 15837671, 15857161, 15876629, 15896113, 15915607, 15935077,
            15954563, 15974039, 15993521, 16012987, 16032469, 16051949, 16071437, 16090931, 16110401,
            16129873, 16149409, 16168861, 16188313, 16207843, 16227287, 16246753, 16266251, 16285729,
            16305203, 16324691, 16344179, 16363651, 16383137, 16402609, 16422079, 16441583, 16461043,
            16480537, 16500007, 16519487, 16538969, 16558453, 16577927, 16597433, 16616903, 16636369,
            16655851, 16675333, 16694809, 16714303, 16733767, 16753249, 16772741, 16792229, 16811699,
            16831201, 16850657, 16870129, 16889617, 16909091, 16928579, 16948073, 16967539, 16987049,
            17006519, 17025977, 17045459, 17064941, 17084443, 17103901, 17123383, 17142859, 17162363,
            17181821, 17201323, 17220781, 17240263, 17259773, 17279243, 17298733, 17318201, 17337673,
            17357143, 17376629, 17396107, 17415613, 17435083, 17454587, 17474029, 17493521, 17512991,
            17532467, 17551979, 17571431, 17590931, 17610401, 17629897, 17649353, 17668877, 17688317,
    })(pxtc = ts.pxtc || (ts.pxtc = {}));
})(ts || (ts = {}));
// TODO: enable reference so we don't need to use: (pxt as any).py
        ...
        function createSymbolInfo(typechecker, qName, stmt) {
            ...
            if (kind != 0 /* SymbolKind.None */) {
                ...
                let r = {
                    ...
                    pkgs,
                    extendsTypes,
                    isStatic: (_a = decl.modifiers) === null || _a === void 0 ? void 0 : _a.some(m => m.kind === ts.SyntaxKind.StaticKeyword),
    ...
    (function (pxtc) {
        ...
        (function (service_1) {
            ...
            class Host {
                ...
                log(s) { pxt.log("LOG", s); }
                trace(s) { pxt.log("TRACE", s); }
                error(s) { pxt.error("ERROR", s); }
                useCaseSensitiveFileNames() { return true; }
            }
            ...
            const operations = {
                ...
                apiSearch: v => {
                    ...
                    const computeBlockString = (symbol, skipParent = false, paramMap) => {
                        var _a, _b, _c, _d, _e;
                        const toolboxParent = ((_a = symbol.attributes) === null || _a === void 0 ? void 0 : _a.toolboxParent) || ((_b = symbol.attributes) === null || _b === void 0 ? void 0 : _b.duplicateWithToolboxParent);
                        const toolboxArgument = ((_c = symbol.attributes) === null || _c === void 0 ? void 0 : _c.toolboxParentArgument) || ((_d = symbol.attributes) === null || _d === void 0 ? void 0 : _d.duplicateWithToolboxParentArgument);
                        ...
                        if ((_e = symbol.attributes) === null || _e === void 0 ? void 0 : _e._def) {
                            let block = [];
                            ...
                            for (let part of blockDef.parts) {
                                switch (part.kind) {
                                    ...
                                    case "param":
                                        ...
                                        let valueString = (actualParam === null || actualParam === void 0 ? void 0 : actualParam.defaultValue)
                                            || part.varName
                                            || (actualParam === null || actualParam === void 0 ? void 0 : actualParam.actualName)
                                            || part.name;
                                        if (paramMap === null || paramMap === void 0 ? void 0 : paramMap["*"]) {
                                            valueString = paramMap["*"];
                                            delete paramMap["*"];
                                        }
                                        else if (paramMap === null || paramMap === void 0 ? void 0 : paramMap[actualParam.definitionName]) {
                    ...
                    const computeDropdownOptionsString = (symbol) => {
                        let options = [];
                        if (symbol.parameters) {
                            symbol.parameters.forEach(param => {
                                ...
                                if (param.isEnum && param.type) {
                                    ...
                                    if (enumInfo && enumInfo.kind === 6 /* ts.pxtc.SymbolKind.Enum */) {
                                        // Find enum members
                                        Object.keys(blockInfo.apis.byQName).forEach(qName => {
                                            const member = blockInfo.apis.byQName[qName];
                                            if (member.kind === 7 /* ts.pxtc.SymbolKind.EnumMember */ &&
                                ...
                                if (typeInfo && typeInfo.attributes.fixedInstances) {
                                    ...
                                    const syms = pxt.Util.values(blockInfo.apis.byQName).filter(sym => sym.kind === 4 /* ts.pxtc.SymbolKind.Variable */ &&
                                ...
                                if (symbol.attributes.constantShim) {
                                    const syms = pxt.Util.values(blockInfo.apis.byQName).filter(sym => sym.attributes.blockIdentity === symbol.qName);
                                    syms.forEach(sym => {
                                        const k = sym.attributes.block || sym.attributes.blockId || sym.name;
                                        options.push(k);
                                    });
                ...
                homeSearch: v => {
                    ...
                    const searchKey = searchSet.map((h) => h.id).join("\n");
...
"""`;
            ...
            function getSnippet(context, fn, decl, python, recursionDepth = 0) {
                ...
                if (includeParentSnippet && element.attributes.toolboxParent && recursionDepth === 0) {
                    ...
                    parentSnippet = getSnippet(Object.assign(Object.assign({}, context), { parameterOverride }), parentFn, service.lastApiInfo.decls[parentFn.qName], python, recursionDepth + 1);
