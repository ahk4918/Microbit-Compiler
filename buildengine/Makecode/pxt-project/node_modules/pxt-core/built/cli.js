...
function uploadCoreAsync(opts) {
    ...
    if ((_c = pxt.appTarget.appTheme) === null || _c === void 0 ? void 0 : _c.extendScriptPage)
...
function ghpInitAsync() {
    ...
    return ghpGitAsync("checkout", "gh-pages")
        ...
        .catch((e) => ghpGitAsync("checkout", "--orphan", "gh-pages"))
...
function ghpPushAsync(builtPackaged, minify = false) {
    let repoName = "";
    return ghpInitAsync()
...
async function internalBuildTargetAsync(options = {}) {
...
async function ciBuildInfoAsync() {
    if (isTravis())
        return travisInfo();
    else if (isGithubAction())
        return await githubActionInfoAsync();
    else if (isAzurePipelines())
        return travisInfo(); // azure pipelines uses same info
    else {
        // local build
        return {
            ci: "local",
    ...
    async function githubActionInfoAsync() {
...
function setCiBuildInfo(tag, branch, commit, repoSlug) {
    // travis
...
function saveThemeJson(cfg, localDir, packaged) {
    ...
    if (packaged) {
        cfg = replaceStaticImagesInJsonBlob(cfg, fn => path.join('./docs', fn).replace(/\\/g, "/"));
    }
    else if (!localDir) {
        cfg = replaceStaticImagesInJsonBlob(cfg, fn => uploadedArtFileCdnUrl(fn));
    }
    ...
    if (theme.homeScreenHero && typeof theme.homeScreenHero != "string") {
        ...
        if (heroBannerCard.title)
            targetStrings[heroBannerCard.title] = heroBannerCard.title;
        if (heroBannerCard.description)
            targetStrings[heroBannerCard.description] = heroBannerCard.description;
        if (heroBannerCard.buttonLabel)
    ...
    if ((_c = (_b = (_a = cfg.runtime) === null || _a === void 0 ? void 0 : _a.functionsOptions) === null || _b === void 0 ? void 0 : _b.extraFunctionEditorTypes) === null || _c === void 0 ? void 0 : _c.length) {
    ...
    if (typeof ((_d = cfg.appTheme) === null || _d === void 0 ? void 0 : _d.assetEditor) === "object") {
    ...
    if ((_f = (_e = cfg.appTheme) === null || _e === void 0 ? void 0 : _e.pxtJsonOptions) === null || _f === void 0 ? void 0 : _f.length) {
    ...
    if (nodeutil.fileExistsSync("targetconfig.json")) {
        ...
        if (targetConfig === null || targetConfig === void 0 ? void 0 : targetConfig.galleries) {
...
`;
            ...
            nodeutil.writeFileSync(path.join(docsRoot, "docs/projects.md"), `# Projects
...
`, { encoding: "utf8" });
        ...
        const multiplayerGames = (_g = targetConfig === null || targetConfig === void 0 ? void 0 : targetConfig.multiplayer) === null || _g === void 0 ? void 0 : _g.games;
        ...
        const kioskGames = (_h = targetConfig === null || targetConfig === void 0 ? void 0 : targetConfig.kiosk) === null || _h === void 0 ? void 0 : _h.games;
        ...
        const approvedRepoLib = (_j = targetConfig === null || targetConfig === void 0 ? void 0 : targetConfig.packages) === null || _j === void 0 ? void 0 : _j.approvedRepoLib;
        for (const [extension, repoData] of Object.entries(approvedRepoLib !== null && approvedRepoLib !== void 0 ? approvedRepoLib : {})) {
            for (const tag of ((_k = repoData.tags) !== null && _k !== void 0 ? _k : [])) {
                targetStrings[`{id:extension-tag}${tag}`] = tag;
            }
        }
        const builtinExtensionLib = (_l = targetConfig === null || targetConfig === void 0 ? void 0 : targetConfig.packages) === null || _l === void 0 ? void 0 : _l.builtinExtensionsLib;
        for (const [extension, repoData] of Object.entries(builtinExtensionLib !== null && builtinExtensionLib !== void 0 ? builtinExtensionLib : {})) {
            for (const tag of ((_m = repoData.tags) !== null && _m !== void 0 ? _m : [])) {
                targetStrings[`{id:extension-tag}${tag}`] = tag;
            }
        }
        const hardwareOptions = targetConfig === null || targetConfig === void 0 ? void 0 : targetConfig.hardwareOptions;
        for (const opt of (hardwareOptions !== null && hardwareOptions !== void 0 ? hardwareOptions : [])) {
...
async function buildSemanticUIAsync(parsed) {
    ...
    await Promise.all(subwebapp_1.SUB_WEBAPPS.filter(app => app.buildCss).map(app => generateReactCommonCss(app.name)));
...
function updateDefaultProjects(cfg) {
    ...
    if (!cfg.tsprj && cfg.blocksprj) {
        let notBlock = (s) => !U.endsWith(s, ".blocks");
...
function updateColorThemes(cfg) {
    var _a;
    ...
    for (const themeFile of themeFiles) {
        ...
        for (const overrideFile of (_a = themeData.overrideFiles) !== null && _a !== void 0 ? _a : []) {
            if (!overrideFile) {
                // Skip empty entries
                continue;
            }
        ...
        if (cfg.colorThemeMap[theme.id]) {
            ...
            cfg.colorThemeMap[theme.id] = Object.assign(Object.assign(Object.assign({}, existingTheme), theme), { colors: Object.assign(Object.assign({}, existingTheme.colors), theme.colors) });
        }
        else {
...
async function buildTargetCoreAsync(options = {}) {
    var _a;
    let cfg = readLocalPxTarget();
    ...
    const packageDirs = (((_a = pxt.appTarget.staticpkgdirs) === null || _a === void 0 ? void 0 : _a.extensions) || pxt.appTarget.bundleddirs).map(dir => path.resolve(dir));
    const rootDir = process.cwd();
    await buildWebStringsAsync();
    if (!options.quick)
        await internalGenDocsAsync(false, true);
    if (pxt.appTarget.cacheusedblocksdirs && !options.quick)
    ...
    await forEachBundledPkgAsync(async (pkg, dirname) => {
        ...
        if (isPrj && ((_a = pxt.appTarget.compile) === null || _a === void 0 ? void 0 : _a.hasHex)) {
            ...
            if (!options.packaged)
                return;
            if (pxt.appTarget.staticpkgdirs && pxt.appTarget.staticpkgdirs.base.indexOf(dirname) === -1)
                return;
...
function fillInCompilerExtension(cfg) {
    ...
    for (const docFolder of docFolders) {
        for (const f of nodeutil.allFiles(docFolder, { maxDepth: 8 })) {
            ...
            if (/\.(md|html)$/.test(f)) {
                ...
                let html = "";
                if (U.endsWith(f, ".md")) {
                    const md = nodeutil.resolveMd(".", pathUnderDocs.slice(0, -3), fileData);
...
function serveAsync(parsed) {
    ...
    let localToken = exports.globalConfig.localToken;
    if (!fs.existsSync("pxtarget.json")) {
    ...
    return (justServe ? Promise.resolve() : buildAndWatchTargetAsync(includeSourceMaps, rebundle))
        ...
        https: parsed.flags["https"] || false,
    }));
}
...
class Host {
    ...
    readFile(module, filename, skipAdditionalFiles) {
    ...
    getHexInfoAsync(extInfo) {
        if (process.env["PXT_LOCAL_DOCKER_TEST"] === "yes") {
            ...
            const mappedFiles = Object.keys(compileReq.replaceFiles).map(k => {
                return {
                    name: k.replace(/^\/+/, ""),
...
function initAsync(parsed) {
    ...
    let configMap = JSON.parse(files[pxt.CONFIG_NAME]);
    let initPromise = Promise.resolve();
    if (!parsed.flags["useDefaults"]) {
        initPromise = U.promiseMapAllSeries(["name", "description", "license"], f => queryAsync(f, configMap[f])
            .then(r => {
            configMap[f] = r;
        })).then(() => { });
    }
...
function downloadPlaylistsAsync(parsed) {
    const fn = (parsed.args[0] || "playlists.json");
...
exports.downloadPlaylistsAsync = downloadPlaylistsAsync;
async function validateAndFixPkgConfig(parsed) {
    ...
    function validateFileList(field, files) {
...
exports.validateAndFixPkgConfig = validateAndFixPkgConfig;
function downloadDiscourseTagAsync(parsed) {
    var _a;
    const tag = parsed.args[0];
    if (!tag)
        U.userError("Missing tag");
    const out = parsed.flags["out"] || "temp";
    const outmd = parsed.flags["md"];
    ...
    const md = outmd && fs.readFileSync(outmd, { encoding: "utf8" });
    const title = md && ((_a = /^# (.*)$/m.exec(md)) === null || _a === void 0 ? void 0 : _a[1]);
    nodeutil.mkdirP(out);
    let n = 0;
    let newcards = 0;
    let cards = [];
...
`;
    ...
    function downloadImageAsync(id, topic, url) {
        return pxt.Util.requestAsync({
            ...
            if (resp.buffer) {
                const m = /image\/(png|jpeg|gif)/.exec(resp.headers["content-type"]);
                if (!m) {
                    ...
                    nodeutil.writeFileSync(localifn, new Buffer(resp.buffer));
                    if (/\.(jpg|png)/.test(ifn))
                        topic.imageUrl = ifn;
...
exports.downloadDiscourseTagAsync = downloadDiscourseTagAsync;
...
async function testSnippetsAsync(snippets, re, pyStrictSyntaxCheck) {
    ...
    let ignoreCount = 0;
    const ghExtensionCache = {};
    const compileOptsCache = {};
    const successes = [];
    const failures = [];
    const addSuccess = (s) => {
        successes.push(s);
    };
    const addFailure = (f, infos) => {
        failures.push({
            filename: f,
            diagnostics: infos
        });
    ...
    const sortedSnippets = snippets.sort((a, b) => getCompileOptsKey(a.packages).localeCompare(getCompileOptsKey(b.packages)) || a.name.localeCompare(b.name));
    ...
    await U.promiseMapAllSeries(sortedSnippets, async (snippet) => {
        var _a;
        ...
        if (snippet.ext == "json") {
            try {
                ...
                addFailure(snippet.src, [createKsDiagnostic(`invalid JSON: ${e.message}`, fn)]);
            }
            return;
        }
        ...
        try {
            ...
            if (isPy) {
                ...
                const { ast } = resp;
                const { outfiles, diagnostics, success } = pxtc.service.performOperation("py2ts", { options: opts });
                resp = {
                    outfiles,
                    success,
                    diagnostics,
            ...
            if (!resp.success || ((_a = resp.diagnostics) === null || _a === void 0 ? void 0 : _a.length)) {
            ...
            console.dir(e);
            addFailure(snippet.src, [createKsDiagnostic(e.message, fn)]);
        }
    });
    ...
    function tsServiceCompile(options, retryOnError) {
        ...
        return resp;
        function tryCompile(options) {
            const resp = pxtc.service.performOperation("compile", { options });
            if (resp.errorMessage) {
                resp.success = false;
                if (!resp.diagnostics) {
                    resp.diagnostics = [];
                }
                resp.diagnostics.push(createKsDiagnostic(resp.errorMessage));
            }
            return resp;
        }
    }
    async function getCompileResources(snippetFiles, pkgs) {
    ...
    function createKsDiagnostic(message, fn = "?", code = 4242) {
        return {
            code,
    ...
    function getCompileOptsKey(pkgs) {
        const keys = Object.keys(pkgs).sort();
        return keys.reduce((prv, curr) => `${prv},${curr}:${pkgs[curr]}`, "");
    }
    function pyComparisonString(s) {
        return s.split("\n")
...
function dumpmemAsync(c) {
    ensurePkgDir();
    return mainPkg.loadAsync()
        .then(() => gdb.dumpMemAsync(c.args));
}
async function buildDalDTSAsync(c) {
    forceLocalBuild = true;
    ...
    return prepBuildOptionsAsync(buildOpts.mode, false, buildOpts.ignoreTests)
        ...
        switch (buildOpts.mode) {
            case BuildOption.GenDocs: {
                ...
                const pxtsnippet = pxt.Util.jsonTryParse(mainPkg.readFile('pxtsnippets.json'));
...
async function staticpkgAsync(parsed) {
    ...
    const locsSrc = parsed.flags["locs-src"];
...
function buildJResSpritesDirectoryAsync(dir) {
    ...
    const metaInfo = nodeutil.readJson(metaInfoPath);
    const jresources = {};
    ...
    function closestColor(buf, pix, alpha = true) {
    ...
    function processImage(basename, pngName, jsonName, standalone) {
        ...
        let sheet = PNG.sync.read(fs.readFileSync(pngName));
        let imgIdx = 0;
        // add alpha channel
        ...
        function processAnimation(name, frames, flippedHorizontal = false) {
            jresources[name] = {
...
async function buildShareSimJsAsync(parsed) {
    ...
    const outdir = parsed.flags["output"] || path.join(cwd, "docs", "static", "builtjs");
    nodeutil.mkdirP(outdir);
    const outputLocation = path.join(outdir, `${id}v${pxt.appTarget.versions.target}.json`);
    fs.writeFileSync(outputLocation, JSON.stringify(builtJsInfo));
    process.chdir(cwd);
    console.log(`saved prebuilt ${id} to ${outputLocation}`);
}
exports.buildShareSimJsAsync = buildShareSimJsAsync;
async function buildCoreDeclarationFiles(parsed) {
    var _a, _b;
    const shareId = (_a = (parsed && parsed.flags["shareid"])) === null || _a === void 0 ? void 0 : _a.trim();
    ...
    const host = shareId ? new Host() : new SnippetHost("decl-build", { "main.ts": "" }, Object.assign({}, blocksprjConfig.dependencies));
    ...
    opts.tsCompileOptions = (_b = opts.tsCompileOptions) !== null && _b !== void 0 ? _b : {};
    ...
    for (const file of tsProg.getSourceFiles()) {
        if (file.fileName.endsWith(".d.ts")) {
            ...
            writeDts(file.fileName, file.getFullText());
        }
    }
    // generated via build
    tsProg.emit(
    /** targetSourceFile **/ undefined, (fileName, data) => {
...
exports.buildCoreDeclarationFiles = buildCoreDeclarationFiles;
function gendocsAsync(parsed) {
    const docs = !!parsed.flags["docs"];
    const locs = !!parsed.flags["locs"];
    const fileFilter = parsed.flags["files"];
...
function extractBufferAsync(buf, outDir) {
    const oneFile = (src, editor) => {
        let files = {};
        files["main." + (editor || "td")] = src || "";
        return files;
    };
    const unpackHexAsync = (buf) => pxt.cpp.unpackSourceFromHexAsync(buf)
        .then(data => {
        if (!data)
            return null;
        if (!data.meta)
    ...
    return Promise.resolve()
        ...
        if (Array.isArray(json.scripts)) {
            ...
            json.projects = json.scripts.map((scr) => ({
        ...
        if (json[pxt.CONFIG_NAME]) {
            ...
            let cfg = pxt.Package.parseAndValidConfig(json[pxt.CONFIG_NAME]);
            let files = json;
            json = {
                projects: [{
                        name: cfg.name,
...
function internalCheckDocsAsync(compileSnippets, re, fix, pycheck) {
    ...
    nodeutil.allFiles("docs", { ignoredFileMarker: ignoredFoldersKey })
        ...
        let newmd = md.replace(/]\((\/static\/[^)]+?)\.(png|jpg)(\s+"[^"]+")?\)/g, (m, p, ext, comment) => {
    ...
    function addSnippet(snippet, entryPath, snipIndex, src) {
    ...
    if (fs.existsSync("targetconfig.json")) {
        const targeConfig = nodeutil.readJson("targetconfig.json");
    ...
    while (todo.length) {
        ...
        let md = urls[entrypath] || nodeutil.resolveMd(docsRoot, entrypath);
        if (!md) {
                            ...
                            for (let url of urls) {
                                ...
                                if (tutorial.code.some(tut => tut.indexOf("namespace") !== -1)
                                    ...
                                    tutorial.steps
                                        ...
                                        addSnippet(snippet, `tutorial/${card.name}-${indexInStep}step`, stepIndex, url + (indexInStep));
                                    }));
                                }
                                else {
                                    addSnippet({
...
async function upgradeCardsAsync() {
    ...
    mds.forEach(({ filename, content }) => {
        ...
        const updated = content.replace(/```codecard([^`]+)```/g, (m, c) => {
...
function internalCacheUsedBlocksAsync() {
    ...
    for (let i = 0; i < mdPaths.length; i++) {
        ...
        const pkgs = pxt.tutorial.mergeTutorialDependencies(blocksprjConfig.dependencies, tutorialDeps);
        tutorial.pkgs = pkgs;
        tutorial.path = path;
    ...
    return U.promiseMapAll(Object.keys(tutorialInfo), (hash) => {
        ...
        if (isPy) {
            ...
            inFiles = Object.assign({ [pxt.MAIN_TS]: "", [pxt.MAIN_PY]: "", [pxt.MAIN_BLOCKS]: "" }, extra);
        }
        else {
                ...
                if ((decompiled === null || decompiled === void 0 ? void 0 : decompiled.length) > 0) {
                    ...
                    for (let i = 0; i < decompiled.length; i++) {
                        ...
                        blocksXml.replace(blockIdRegex, (m0, m1) => {
                            if (!builtInfo.snippetBlocks[snippetHash])
...
function getCodeSnippets(fileName, md, baseConfig) {
    const supported = {
        "blocks": "ts",
        "block": "ts",
        "typescript": "ts",
    ...
    const tutorialDeps = snippets.filter(snip => snip.type == "package")
        ...
        for (let line of lines) {
            ...
            if (i < 0)
                ...
                acc[line.substring(0, i).trim()] = line.substring(i + 1);
        }
        return acc;
    }, {});
...
function testGithubPackagesAsync(parsed) {
    ...
    const filterRx = parsed.args["filter"] && new RegExp(parsed.flags["filter"]);
    const targetConfig = nodeutil.readJson("targetconfig.json");
    ...
    function resolveTagAsync(fullname) {
    ...
    function buildAsync(pkgpgh, tag) {
...
function blockTestsAsync(parsed) {
    ...
    return writeBlockTestJSONAsync()
        ...
        args
    }), (e) => console.log("Skipping blocks tests: " + e.message));
            ...
            if (fs.existsSync(path.resolve(configPath)) && nodeutil.existsDirSync(dirPath)) {
                return readFileAsync(configPath, "utf8")
                    ...
                    return readDirAsync(dirPath)
                        .then(files => U.promiseMapAll(files.filter(f => U.endsWith(f, ".blocks") && f != pxt.MAIN_BLOCKS), fn => readFileAsync(path.join(dirPath, fn), "utf8")
                        .then((contents) => testFiles.push({ testName: fn, contents }))));
                })
                    .then(() => { return { packageName, testFiles }; });
            }
            return Promise.resolve(undefined);
        }))
            .then((allCases) => allCases.filter(f => !!f && f.testFiles.length && f.packageName));
    }
...
`);
    ...
    function advancedCommand(name, help, callback, argString, onlineHelp = false) {
        p.defineCommand({ name, help, onlineHelp, argString, advanced: true }, callback);
    }
}
function handleCommandAsync(args, preApply = () => Promise.resolve()) {
...
function mainCli(targetDir, args = process.argv.slice(2)) {
    ...
    const versions = pxt.appTarget.versions || { target: "", pxt: "" };
